...
// ---- Chat analytics (charts/tables generated by chat) ---------------------
// Defined here (not in api.ts) so both api.ts and chat's persisted-message
// type below can import them without an api.ts <-> types.ts import cycle.
...
export interface ChatMessageOut {
  ...
  content: string;
  created_at: string;
  // Present when this turn was backed by a generated analytics query, so
  // reloading a session (GET /chat/sessions/{id}) restores the same
  // table/chart the user saw live, not just the answer text.
