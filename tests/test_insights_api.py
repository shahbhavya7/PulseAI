...
Each test uses a fresh isolated user (via X-User-Id) so aggregates are
deterministic. The model + embeddings are monkeypatched — no OpenAI key needed.
"""
...
def _analyze_ticket(client: TestClient, headers: dict[str, str], text: str) -> None:
    data = (f"text\n{text}\n").encode()
...
def test_upload_to_summary_to_stats(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _analyze_ticket(client, headers, f"crash and double charge {uuid4().hex}")
    week = iso_week()

    # Generate the weekly summary (fed only this week's issues).
    gen = client.post(f"/summaries/{week}", headers=headers)
    ...
    # Read it back.
    got = client.get(f"/summaries/{week}", headers=headers)
    assert got.status_code == 200
    assert got.json()["narrative"] == body["narrative"]

    # Dashboard stats (SQL), scoped to this user + week.
    stats = client.get(f"/stats?week={week}", headers=headers).json()
...
def test_stats_filter_needs_manual_review(
    ...
    headers = {"X-User-Id": _new_user()}
    _analyze_ticket(
        client, headers, f"the checkout page keeps failing on submit case {uuid4().hex}"
    )
    week = iso_week()
    # confidence 0.9 → not flagged for review, so this filter yields zero.
    stats = client.get(f"/stats?week={week}&needs_manual_review=true", headers=headers).json()
    assert stats["total_issues"] == 0


def test_summary_no_issues_returns_404(client: TestClient) -> None:
    headers = {"X-User-Id": _new_user()}
    resp = client.post("/summaries/2000-W01", headers=headers)
...
def test_embeddings_written_not_marked_reembed(
    ...
    monkeypatch.setattr(pipeline, "get_vector_store", _FakeStore)
    headers = {"X-User-Id": _new_user()}
    _analyze_ticket(client, headers, f"the search results load very slowly for query {uuid4().hex}")
    ...
    with get_sessionmaker()() as db:
        rows: Any = db.execute(
            ...
            .where(Ticket.owner_id == UUID(headers["X-User-Id"]))
        ).all()
